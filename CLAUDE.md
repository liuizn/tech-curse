...
- `PaymentRepository.UpdateAsync` marca só o pagamento como modificado (`Entry(payment).State = Modified`), não suas navegações — processar/estornar nunca reescreve linhas de matrícula, curso ou aluno.
...
- O cadastro de um `Student` (pelo registro público ou pelo `/Auth/users`) cria também a entidade `Student` ligada ao usuário. A criação é compensada, não transacional — o provider em memória dos testes não suporta transação: se a atribuição da role ou a gravação do perfil falharem, o `AuthService` apaga o usuário recém-criado e relança o erro. O 409 vale só para perfil antigo sem usuário (`Student` com aquele e-mail mas sem `IdentityUser` correspondente); e-mail que já pertence a um usuário existente responde 422 `DuplicateEmail`.
...
- **Soft delete**: `Student` tem global query filter (`!s.IsDeleted`) no `OnModelCreating`. Consultas de `Payment` que navegam até `Student` chamam `IgnoreQueryFilters()`; `Enrollment` e `Course` não têm filtro de soft delete — ver a seção "Soft delete: decisão tomada".
...
- **O publish do `Dockerfile` não usa `--no-restore`.** O restore roda num passo próprio com cache mount (`/root/.nuget/packages`), mas o cache de camadas do CI pode trazer esse passo pronto num runner novo, com o cache mount vazio; aí um publish `--no-restore` falha com `NETSDK1064` (pacote do `project.assets.json` ausente) — aconteceu no merge do #40. Sem a flag, o publish restaura só o que faltar e é um no-op quando os pacotes estão lá. As flags de RID continuam nos **dois** passos: com `PublishReadyToRun=true` e nenhum RuntimeIdentifier declarado, o restore não grava o alvo no `project.assets.json` nem baixa os runtime packs do crossgen (`NETSDK1047`/`NETSDK1112`), por isso `--use-current-runtime` aparece no restore e no publish, mais `--no-self-contained` no publish (a imagem final é a runtime `aspnet`; declarar RID sozinho poderia virar self-contained e inchar a imagem).
