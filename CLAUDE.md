...
- **Cache**: queries leem/gravam em Redis via `ICacheService` com chaves prefixadas (`payments:list:`, `payments:item:`, `payments:student:`, `payments:enrollment:`, …). Todo command que muta dados chama `RemoveByPrefixAsync` para cada prefixo afetado — ao adicionar uma nova query com cache, adicione a invalidação correspondente nos commands. As chaves de cache de pagamento ficam em `ChavesDeCachePagamento` (prefixo `payments:v2:`); mudar o formato do `PaymentOutputDto` exige subir a versão do prefixo. `PaymentRepository.UpdateAsync` marca só o pagamento como modificado (`Entry(payment).State = Modified`), não suas navegações — processar/estornar nunca reescreve linhas de matrícula, curso ou aluno.
